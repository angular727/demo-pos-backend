...
bankRouter
 ...
 .post(cors.corsWithOptions, verifyUser, (req, res, next) => {
    const bankData ={...req.body, user: req.user._id, vendorId: req.user.vendorId}
  ...
  .put(cors.corsWithOptions, verifyUser, (req, res, next) => {
    res.statusCode = 403;
    res.end("PUT operation not supported on /Bank");
  });
...
try {
  const totalComplaints = await Bank.countDocuments(find);
  const totalPages = Math.ceil(totalComplaints / pageSize);

  const complaints = await Bank.find(find)
    .sort({ createdAt: -1 })
...
bankRouter
  ...
  .get(cors.cors, (req, res, next) => {
    Bank.findById(req.params.productId)
  ...
  .post(cors.corsWithOptions,  (req, res, next) => {
    res.statusCode = 403;
    res.end(
      "POST operation not supported on /Bank/" + req.params.productId
    );
  })
  ...
  .delete(cors.corsWithOptions, (req, res, next) => {
    Bank.findByIdAndRemove(req.params.productId)
      .then(
        (resp) => {
          ////////////////////////////emit total 
