...
attendanceRouter
 ...
 .post(cors.corsWithOptions, verifyUser, (req, res, next) => {
    const attendanceData ={...req.body, user: req.user._id, vendorId: req.user.vendorId}
...
try {
  const totalComplaints = await Attendance.countDocuments(find);
  const totalPages = Math.ceil(totalComplaints / pageSize);

  const units = await Attendance.find(find)
    .sort({ createdAt: -1 })
...
attendanceRouter
  ...
  .get(cors.cors, (req, res, next) => {
    Attendance.findById(req.params.productId)
  ...
  .post(cors.corsWithOptions,  (req, res, next) => {
    res.statusCode = 403;
    res.end(
      "POST operation not supported on /Attendance/" + req.params.productId
    );
  })
  ...
  .delete(cors.corsWithOptions, (req, res, next) => {
    Attendance.findByIdAndRemove(req.params.productId)
      .then(
        (resp) => {
          ////////////////////////////emit total 
