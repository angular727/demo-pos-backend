...
employeeRouter
 ...
 .post(cors.corsWithOptions, verifyUser, (req, res, next) => {
  const employeeData = {...req.body, user: req.user._id, vendorId: req.user.vendorId}
...
try {
  const totalComplaints = await Employees.countDocuments(find);
  const totalPages = Math.ceil(totalComplaints / pageSize);

  const units = await Employees.find(find)
    .sort({ createdAt: -1 })
...
employeeRouter
  ...
  .get(cors.cors, (req, res, next) => {
    Employees.findById(req.params.productId)
  ...
  .post(cors.corsWithOptions,  (req, res, next) => {
    res.statusCode = 403;
    res.end(
      "POST operation not supported on /Employees/" + req.params.productId
    );
  })
  ...
  .delete(cors.corsWithOptions, (req, res, next) => {
    Employees.findByIdAndRemove(req.params.productId)
      .then(
        (resp) => {
          ////////////////////////////emit total 
